...
- **`actions/setup-node@v5`** — pinned to the current major so the Actions runtime (Node 24) is happy. We deliberately **omit** `cache: npm`: it looks for a `package-lock.json`/`yarn.lock` in the repo, and this repo has none — we install a single global CLI, not local dependencies. A `cache: npm` line here fails the job with *"Dependencies lock file is not found"*.
- **`actions/checkout@v5`** — same reason: `@v5` runs on Node 24, `@v4` still declares Node 20 and now emits a deprecation warning.
...
| `command not found: api-catalog` | Node version too old, or install step skipped | Ensure `actions/setup-node@v5` ran with `node-version: "22"` and that the `Install API Catalog CLI` step executed |
| `Dependencies lock file is not found` on `setup-node` | `cache: npm` is set but the repo has no `package-lock.json`/`yarn.lock` | Remove `cache: npm` — we install a global CLI, not local deps, so there's nothing to cache |
| Deprecation warning: *"Node 20 is being deprecated…"* | Using older action majors (`@v4`) that still declare a Node 20 runtime | Bump `actions/checkout` and `actions/setup-node` to `@v5` (Node 24 runtime) |
